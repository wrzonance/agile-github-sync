...
def test_prev_does_not_advance_when_gh_write_is_skipped():
    ...
    queue = _Queue()
    with patch("agilesync.syncers.metadata_sync.ghproject.set_project_date", return_value=False) as write_mock:
...
def test_prev_advances_when_gh_write_succeeds():
    ...
    queue = _Queue()
    with patch("agilesync.syncers.metadata_sync.ghproject.set_project_date", return_value=True) as write_mock:
        sync_dates({}, True, issue, card, pitem, _field_meta(), state, queue)
    write_mock.assert_called_once()
...
def test_prev_advances_when_new_already_matches_gh_no_write_attempted():
    ...
    queue = _Queue()
    with patch("agilesync.syncers.metadata_sync.ghproject.set_project_date") as write_mock:
        sync_dates({}, True, issue, card, pitem, _field_meta(), state, queue)
...
def test_prev_does_not_advance_when_apply_is_false():
    ...
    state = _issues_state(issue["url"], start="2026-01-01")
    queue = _Queue()
    with patch("agilesync.syncers.metadata_sync.ghproject.set_project_date", return_value=True):
